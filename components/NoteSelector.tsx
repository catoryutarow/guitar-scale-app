import { ROOT_NOTES, ENHARMONIC_PAIRS, getPitchClass } from '@/lib/scales';
...
export default function NoteSelector({ selectedNote, onSelectNote }: NoteSelectorProps) {
  ...
  // 選択中の音のピッチを取得
  const selectedPitch = getPitchClass(selectedNote);
    ...
    <div>
      <div className="flex flex-wrap gap-2 justify-center mb-4">
        {ROOT_NOTES.map(({ note, display }) => {
          ...
          const isSelected = notePitch === selectedPitch;
      ...
      {hasEnharmonic && (
        <div className="flex justify-center">
          ...
          >
            ♯系/♭系を切り替え: {selectedNote} ⇄ {enharmonicNote}
          </button>
        </div>
      )}
    </div>
